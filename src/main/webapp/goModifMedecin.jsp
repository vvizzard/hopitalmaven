...
<% 
    ...
    int i;
    for(i = 1; i<specialite.length;i++)if(specialite[i].compareTo(sp)==0)break;
    
    try{
    temp.modifMed(id,nom, prenom, dn, salaire,i);
    }
    catch(Exception e){
        bla = e.getMessage();        
    }
    //Medecin[] tst = temp.insertMed(null,"RA","m","1800/01/01","2000/12/12");
    if(bla.compareTo("")==0||bla.compareTo("Aucun résultat retourné par la requête.")==0)request.getRequestDispatcher("goShow.jsp?id=&nom=&prenom=&dateNaissance1=1800-01-01&dateNaissance2=3000-01-01").forward(request, response);
    else{
